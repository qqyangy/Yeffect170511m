...
var Yeffect={
    ...
    mobileBanner:function(obj1,obj2,num,btnL,btnR,css,time,aniTimer,easingstr){
    ...
    },ImageClipping:function(obj,img,X,fn,maxC,alerta,maxLeng,geshi,isTouxing,boderWidth,boderHeight){
        img=img||"img";
        X=X||"";
        geshi=geshi||"png";
        ...
        function creatIMGbox(img,domimg){
            ...
            if(!ImgboxE){
                ImgboxE=$("<div>");
                ImgboxE.css({"-webkit-backface-visibility":"hidden","-moz-backface-visibility":"hidden","-ms-backface-visibility":"hidden","backface-visibility":"hidden","-webkit-perspective":"1000","-moz-perspective":"1000","-ms-perspective":"1000","perspective":"1000","-webkit-transform":"translate3d(0, 0, 0)","-moz-transform":"translate3d(0, 0, 0)","-ms-transform":"translate3d(0, 0, 0)","transform":"translate3d(0, 0, 0)"});
                ...
                linkL.css({position:"fixed",width:"50px",height:"25x",lineHeight:"25px",textAlign:"center",background:"rgba(0,0,0,0.5)",fontSize:"12px",color:"#fff",bottom:"10px",left:"10px",borderRadius:"20px",textDecoration:"none",zIndex:"99999"});
                linkR.css({position:"fixed",width:"50px",height:"25px",lineHeight:"25px",textAlign:"center",background:"rgba(0,0,0,0.5)",fontSize:"12px",color:"#fff",bottom:"10px",right:"10px",borderRadius:"20px",textDecoration:"none",zIndex:"99999"});
    ...
    },pageScroll: function(boxdiv,minW,offstY,easNum,eastype,ablTime,btn,enterFun,topFun,botmFun,wdom){
        wdom=wdom||window;
        offstY=offstY||0;
        easNum=easNum||5;
        minW=minW||1e3;
        ...
        if(window.addEventListener){
            ...
            var htmlOB=$("html");
            this.event.end(htmlOB,function(ev){
                var deX=ev.deltaX;
                var deY=ev.deltaY;
    ...
    start:function(){
        ...
        call.Ary=[];
        call.leng=0;
        call.then=function(fn){
            var index=this.leng;
            var calla=this;
            function fns(){
                if(calla.Ary.length-1>index){
                    var nextC=calla.Ary[index+1];
                    nextC.data=calla.Ary[index].data;
                    fn(nextC);
                }else{
                    fn({next:function(){},data:calla.Ary[index].data,index:index});
                }
            }
            var newOBJ={next:fns,index:this.leng-1};
            this.Ary.push(newOBJ);
            this.leng++;
            return this;
        }
        call.end=function(){
    ...
    class:function(className,packge){
		    packge=packge||window;
		    if(packge.constructor!=Object){
          packge=window;
        }
        var reg=/^[A-Z]+\w{0,}$/;
        if(!reg.test(className)){
            alert("请以首字符为大写字母的英文或数字组成的字符串为类名！");
            return;
        };
        return {
            constructor:function(fn){
                ...
                var wpar=[];
                packge[className]=fn;
                var d=packge[className];
                d.isClass="Yclass";
                d.parents=[];
                d._callName=className;
                var p = null,o = d.prototype,h = {};
                o.toString=function(){
                    return "[Object "+className+"]";
                };
                return {
                    extends:function(parentClass,par){
                        parentClass=parentClass||false;
                        if(parentClass){
                            if(parentClass["isClass"]!="Yclass"){
                                alert("父类必须为class方法所创建的!");
                                return;
                            };
                            wpar=par||[];
                            if(wpar.constructor!=Array){
                                alert("第二个参数必须以数组形式传入!");
                                return;
                            };

                            /****注入apply方法****/
                            var rega=/^function.[^\{]+\{/;
                            var funSting=fn.toString();
                            var search=rega.exec(funSting)[0];
                            var funStingA=funSting.replace(search,"");
                            search=search.replace("function","function "+className);
                            var rNote = /(\/\/.{0,})|(\/\*(.|\s)*?\*\/)/g; /**匹配单行或多行注释**/
                            funStingA=funStingA.replace(rNote,"");
                            var supreg=/\s_super\(/;
                            if(supreg.test(funStingA)){
                                var spustr="var _superthis=this;function _super(){"
                                    +"var wpar=[];"
                                    +"for(var i=0;i<arguments.length;i++){"
                                    +"wpar.push(arguments[i]);"
                                    +"};"
                                    +"parentClass.apply(_superthis,wpar);"
                                    +"};";
                                funSting=search+spustr+funStingA;
                            }else{
                                funSting=search+"parentClass.apply(this);"+funStingA;
                            };
                            eval(funSting);
                            packge[className]=eval(className);
                            d=packge[className];
                            o = d.prototype;
                            d.isClass="Yclass";
                            d._callName=className;
                            ...
                            d.parents=parentClass.parents.concat([parentClass._callName]);
                            var pr=parentClass.prototype;
                            for(var s in parentClass){
                                if(!d[s]){
                                    d[s]=parentClass[s];
                                }
                            };
                    ...
                    prototype:function(obj){
                        if(obj.constructor!=Object){
                            alert("原型链上的的属性或方法必须以Object的形式传入!");
                            return;
                        };
                        for(n in obj){
                            d.prototype[n]=obj[n];
    ...
    require:function (BasicsUrl,debug) {
      ...
      function require(s) {
        ...
        url=setURlstr(url);
        var fncontent=saveKeyAndFn[url]||{fn:function () {return {};},exports:{}};
        var fn=fncontent.fn;
        if(fn){
          return fn();
        }else{
