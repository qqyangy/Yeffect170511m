define(function () {
    var template = require('./zylibs/zylibs').template;
    //桌面图标
    var appbtnTemp = template(
        '<li class="appbtn<%=classname%>" type="<%=type%>" id="<%=id%>" realid="<%=realid%>" style="top:<%=top%>px;left:<%=left%>px">' +
...
//应用窗口
    var windowTemp = template(
        ...
        '</div>' +
        '<div class="move-bas"></div>'+
        '<div class="title-handle">' +
        '<a class="ha-hide" btn="hide" title="最小化"><b class="hide-b"></b></a>' +
        '<% if(istitlebar){ %>' +
        '<a class="ha-max" btn="max" title="最大化"><b class="max-b"></b></a>' +
        ...
        '<a class="ha-fullscreen" btn="fullscreen" title="全屏">+</a>' +
        //'<% } %>' +
        '<a class="ha-close" btn="close"  title="关闭">×</a>' +
        '</div>' +
...
//文件夹预览
    var folderViewTemp = template(
        ...
        '<% if(mlm){ %>' +
        '<div class="perfect_nine_m_l_m" style="top:<%=mlt%>px;height:20px;display:block;"></div>' +
        '<% }else{ %>' +
        '<div class="perfect_nine_m_l_m" style="top:<%=mlt%>px;height:20px;display:none;"></div>' +
        '<% } %>' +
        '<% if(mlm){ %>' +
        '<div class="perfect_nine_m_l_b" style="top:<%=mlt+20%>px;height:<%=mlb%>px"></div>' +
        '<% }else{ %>' +
        '<div class="perfect_nine_m_l_b" style="top:<%=mlt%>px;height:<%=mlb%>px"></div>' +
        '<% } %>' +
        '</span>' +
        '<span class="perfect_nine_m_r">' +
        '<div class="perfect_nine_m_r_t" style="top:0px;height:<%=mrt%>px"></div>' +
        '<% if(mrm){ %>' +
        '<div class="perfect_nine_m_r_m" style="top:<%=mrt%>px;height:20px;display:block"></div>' +
        '<% }else{ %>' +
        '<div class="perfect_nine_m_r_m" style="top:<%=mrt%>px;height:20px;display:none"></div>' +
        '<% } %>' +
        '<% if(mrm){ %>' +
        '<div class="perfect_nine_m_r_b" style="top:<%=mrt+20%>px;height:<%=mrb%>px"></div>' +
        '<% }else{ %>' +
        '<div class="perfect_nine_m_r_b" style="top:<%=mrt%>px;height:<%=mrb%>px"></div>' +
        '<% } %>' +
        '</span>' +
...
//新建&修改文件夹窗口
    var editFolderDialogTemp = template(
        '<div id="addfolder">' +
        '<a class="folderSelector"><img src="img/ui/default-folder.png"+"<%=src%>"></a>' +
        ...
        '<a class="fcDropdown_item" title="默认"><img class="fcDropdown_img" src="img/ui/default-folder.png"></a>' +
...
//新手帮助提示
    var helpTemp = template(
        ...
        '<span class="h2">欢迎使用智影OS</span>' +
        '<br>下面简单介绍下如何使用，以便您快速上手<br>' +
    ...
    exports.appbtnTemp = appbtnTemp;
    exports.addbtnTemp = addbtnTemp;
    exports.taskTemp = taskTemp;
    exports.windowTemp = windowTemp;
    exports.helpTemp = helpTemp;
    exports.folderViewTemp = folderViewTemp;
    exports.folderWindowTemp = folderWindowTemp;
    exports.editFolderDialogTemp = editFolderDialogTemp;
});